...
Spec::Runner.configure do |config|
  ...
  # customization
  def account_with_cas_mt(opts={})
    @account = opts[:account]
    @account = Account.create! ({:name => opts[:account]})
    config = AccountAuthorizationConfig.new
    cas_url = opts[:cas_url]
    config.auth_type = "cas"
    config.auth_base = cas_url
    if opts[:cas_log_in_url]
      config.log_in_url = opts[:cas_log_in_url]
    else
      config.log_in_url= cas_url
    end
    @account.account_authorization_configs << config
    @account
  end
